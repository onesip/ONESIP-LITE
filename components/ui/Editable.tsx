    ...
    <div className={`relative group ${className}`}>
      ...
      {isEditing && (
        <div className="absolute inset-0 z-50 bg-brand-green-dark/95 p-4 flex flex-col justify-center items-center gap-3 animate-fade-in">
          <p className="text-white text-xs font-bold">输入新图片链接 (URL)</p>
          <input 
            value={url}
            onChange={(e) => setUrl(e.target.value)}
            className="w-full text-xs p-2 rounded text-black"
            placeholder="https://..."
          />
          <div className="flex gap-2">
            <button onClick={handleSave} className="bg-brand-cream text-brand-green-dark px-3 py-1 rounded text-xs font-bold">保存</button>
            <button onClick={() => setIsEditing(false)} className="bg-transparent border border-white text-white px-3 py-1 rounded text-xs">取消</button>
          </div>
        </div>
      )}
    </div>
